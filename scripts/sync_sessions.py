...
Variables d'environnement requises dans .env.local :
    ...
    SANDRA_SPORT_USER_ID       (UUID Auth, à récupérer dans Supabase Dashboard
                                → Authentication → Users → ton email → User UID)

Variables d'environnement optionnelles :
    SANDRA_SPORT_SEANCES_DIR   (override du dossier source)
"""
...
def parse_md_file(path: Path) -> dict[str, Any]:
    ...
    # Fallback date depuis le nom de fichier si absent du frontmatter
    d = meta.get("date")
    if d is None:
        d = path.stem

    # Convertir date en string si c'est un date Python
    ...
    return {
        ...
        "context": post.content.strip() or None,
        "protocol": None,  # enrichi plus tard en parsing fin
        "source": "sandra",
...
def main() -> int:
    ...
    for md_path in md_files:
        ...
        try:
            ...
            print(f"OK   {row['date']} [{status}] {row['title']}")
        except Exception as e:
